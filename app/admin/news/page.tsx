...
export default function Page() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: d } = await (supabase as any).from('news').select('*').order('created_at',{ascending:false})
      setData(d || [])
  ...
  return <AdminLayout adminName={adminName} logoUrl={schoolInfo?.logo_url} schoolName={schoolInfo?.short_name}><NewsClient initialNews={data}/></AdminLayout>
}
