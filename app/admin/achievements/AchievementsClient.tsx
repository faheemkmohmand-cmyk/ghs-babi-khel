...
export default function AchievementsClient({ initialAchievements }: { initialAchievements:Achievement[] }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (editing) {
        const { data, error } = await supabase.from('achievements').update({...form, photo_url} as any).eq('id', editing.id).select().single() as any
        if (error) { toast.error(error.message); return }
        setItems(prev => prev.map(a => a.id===editing.id ? data : a))
        toast.success('Achievement updated ✅')
      } else {
        const { data, error } = await supabase.from('achievements').insert({...form, photo_url} as any).select().single() as any
        if (error) { toast.error(error.message); return }
