...
export default function TimetableClient({ initialSlots, teachers }: { initialSlots:Slot[]; teachers:Teacher[] }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (existing) {
        const { data, error } = await supabase.from('timetable').update(payload).eq('id', existing.id).select().single() as any
        if (error) { toast.error(error.message); return }
        setSlots(prev => prev.map(s => s.id === existing.id ? data : s))
      } else {
        const { data, error } = await supabase.from('timetable').insert(payload).select().single() as any
        if (error) { toast.error(error.message); return }
