  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: s }, { data: t }] = await Promise.all([
        (supabase as any).from('timetable').select('*').order('class').order('day').order('period'),
        (supabase as any).from('teachers').select('id,full_name,subject').eq('status','active').order('full_name'),
      ])
      setSlots(s || [])
      setTeachers(t || [])
