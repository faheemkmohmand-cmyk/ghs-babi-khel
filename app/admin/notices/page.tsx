...
export default function Page() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: d } = await (supabase as any).from('notices').select('*').order('date',{ascending:false})
      setData(d || [])
  ...
  return <AdminLayout adminName={adminName} logoUrl={schoolInfo?.logo_url} schoolName={schoolInfo?.short_name}><NoticesClient initialNotices={data}/></AdminLayout>
}
