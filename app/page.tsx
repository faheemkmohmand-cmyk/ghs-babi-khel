...
export default async function HomePage() {
  ...
  const { data: settingsRows } = await supabase
    .from('school_settings').select('*').order('created_at', { ascending: false }).limit(1)
  ...
  const { data: notices } = await supabase
    .from('notices')
    .select('id,title,type,date,important')
    .eq('published', true)
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-navy-900/95 backdrop-blur-md border-b border-white/5">
        <div className="max-w-7xl mx-auto px-4 flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-2">
            <Link href="/login"
              className="text-white/60 hover:text-white text-sm font-semibold px-4 py-2 rounded-xl border border-white/15 hover:border-white/30 transition-all">
              Login
            </Link>
            <Link href="/signup"
              className="bg-green-900 hover:bg-green-950 text-white text-sm font-bold px-4 py-2 rounded-xl transition-all shadow-md">
              Sign Up
            </Link>
          </div>
        </div>
      ...
      <section className="relative min-h-[92vh] flex items-center overflow-hidden"
        style={{background: 'linear-gradient(135deg, #020810 0%, #0a1628 45%, #014d26 100%)'}}>
      ...
      <footer className="bg-navy-900 text-white py-12">
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid md:grid-cols-3 gap-8 mb-8">
            <div>
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <div className="font-display font-bold text-white">GHS Babi Khel</div>
          ...
          <div className="border-t border-white/8 pt-6 text-center text-white/30 text-xs">
            © {new Date().getFullYear()} Government High School Babi Khel. All rights reserved.
          </div>
        </div>
      </footer>
    </div>
