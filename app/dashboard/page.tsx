...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      Promise.all([
        supabase.from('profiles').select('*').eq('id', session.user.id).maybeSingle() as any,
        supabase.from('students').select('*').eq('user_id', session.user.id).maybeSingle() as any,
        supabase.from('notices').select('id,title,type,date,important').eq('published', true).order('date', {ascending:false}).limit(5) as any,
        supabase.from('exams').select('id,name,start_date,status').eq('status','upcoming').order('start_date',{ascending:true}).limit(4) as any,
      ]).then(([{data:p},{data:s},{data:n},{data:e}]) => {
        setProfile(p)
        setStudent(s)
        setNotices(n || [])
        setExams(e || [])
  ...
  const now = new Date()
  const name = profile?.full_name || (user?.email ? user.email.split('@')[0] : 'Student')
      ...
      <nav className="bg-white border-b border-slate-100 sticky top-0 z-40 shadow-sm">
        <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link href="/" className="text-xs font-bold px-3 py-1.5 rounded-lg border border-green-200 bg-green-50 text-green-800 hover:bg-green-100 transition-all">
              🏫 Home
            </Link>
            ...
            <form action="/auth/signout" method="post">
              <button type="submit" className="text-xs text-slate-400 hover:text-red-500 border border-slate-200 font-semibold px-3 py-1.5 rounded-lg transition-all">
                Sign Out
              </button>
            </form>
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-5">
          <div className="bg-white rounded-3xl border border-slate-100 p-6">
            ...
            {notices.length ? notices.map((n:any)=>(
              <div key={n.id} className="flex items-start gap-2.5 p-3 bg-slate-50 rounded-xl border border-slate-100 mb-2">
          ...
          <div className="bg-white rounded-3xl border border-slate-100 p-6">
            ...
            {exams.length ? exams.map((ex:any)=>{
              ...
              return (
                <div key={ex.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100 mb-2">
                  <div>
