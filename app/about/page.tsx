...
export default async function AboutPage() {
  ...
  const { data: settings } = await (supabase as any).from('school_settings').select('*').limit(1).maybeSingle()
  const { data: teachers } = await (supabase as any).from('teachers').select('full_name,subject,role,photo_url,qualification').eq('status','active').order('full_name').limit(8)
  const { data: achievements } = await (supabase as any).from('achievements').select('*').eq('featured',true).limit(4)
